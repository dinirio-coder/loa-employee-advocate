...
export const getEmployeePayExperience = (employee, options = {}) => {
  ...
  const stateProgram = stateBenefit.applicable && ["FAMILY", "MEDICAL", "COMBINED"].includes(stateBenefit.program?.programType) && stateBenefit.assumedStateOffset > 0
  ...
  if (isParental && model.hasPayData) {
    const target = cents(model.sourcePayValues?.biweeklySalary);
    if (stateProgram) components.push({ label: "State benefit estimate", amount: stateBenefit.assumedStateOffset, explanation: "Planning estimate. State benefits replace part of the paid parental leave estimate during coordination." });
    components.push({ label: "Paid parental leave estimate", amount: cents(Math.max(target - (stateProgram ? stateBenefit.assumedStateOffset : 0), 0)), explanation: "The planning estimate is based on 100% of eligible base pay, subject to applicable policy and payroll processing." });
  } else if (planning) {
  ...
  return {
    ...
    missingInformation: model.missingInformation.filter((message) => !isParental || !/STD breakdown|product classification|source pay period|leave range/i.test(message)).map((message) => message.replace(/source report|source record/gi, "available information").replace(/product classification/gi, "leave type")),
  };
};
